...
fn demo_data_dir() -> std::path::PathBuf {
    ...
    Path::new(&manifest_dir).parent().unwrap().join("data")
}
...
fn test_demo_data_directory_exists() {
    ...
    assert!(
        data_dir.exists(),
        "Demo data directory should exist at {:?}",
        data_dir
    );
}
...
fn test_counterparties_csv_exists_and_valid() {
    ...
    assert!(
        headers.iter().any(|h| h == "counterparty_id"),
        "Should have counterparty_id column"
    );
    assert!(
    ...
    assert!(
        headers.iter().any(|h| h == "rating"),
        "Should have rating column"
    );
    ...
    for result in records {
        ...
        assert!(
            !record.get(0).unwrap().is_empty(),
            "counterparty_id should not be empty"
        );
    }
}
...
fn test_netting_sets_csv_exists_and_valid() {
    ...
    let headers = reader.headers().expect("Should have headers");
    assert!(
        headers.iter().any(|h| h == "netting_set_id"),
        "Should have netting_set_id column"
    );
    assert!(
        headers.iter().any(|h| h == "counterparty_id"),
        "Should have counterparty_id column"
    ...
    assert!(
        headers.iter().any(|h| h == "threshold"),
        "Should have threshold column"
    );
...
fn test_neutryx_toml_exists_and_valid() {
    ...
    assert!(
        config.get("engine").is_some(),
        "Should have [engine] section"
    );
    assert!(
        config.get("pricing").is_some(),
...
fn test_demo_config_toml_exists_and_valid() {
    ...
    assert!(
        ...
        "Should have [gateway] section"
    );
    assert!(
        config.get("workflows").is_some(),
    ...
    assert!(
        ["full", "quick", "custom"].contains(&mode),
        "mode should be full, quick, or custom"
    );
}
...
fn test_holidays_csv_exists_and_valid() {
    ...
    assert!(
        headers.iter().any(|h| h == "calendar"),
        "Should have calendar column"
    );
    assert!(
        headers.iter().any(|h| h == "name"),
        "Should have name column"
    ...
    let calendars: std::collections::HashSet<_> = records
        .iter()
    ...
    assert!(
        calendars.contains("NY") || calendars.contains("US"),
        "Should have NY/US holidays"
    );
