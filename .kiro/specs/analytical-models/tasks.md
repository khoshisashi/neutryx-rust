...
- [ ] 8.2* Dual64 での AD 互換性テスト **[BLOCKED]**
  ...
  - **BLOCKED**: `Dual64` は `num_traits::Float` を実装していないため、現在の `T: Float` トレイト境界では `BlackScholes<Dual64>` / `Bachelier<Dual64>` をインスタンス化できない。AD 互換性を有効にするには、`Float` から `DualNum<f64>` またはカスタム `Scalar` トレイトへのトレイト境界リファクタリングが必要（本仕様の範囲外）。感度検証は有限差分比較テストで代替実施済み。
