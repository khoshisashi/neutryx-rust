...
- [x] 1.1 pricer_kernelからpricer_pricingへのクレート名変更
  ...
  - crates/ディレクトリをpricer_kernelからpricer_pricingにリネーム
...
- [x] 1.3 依存関係グラフの検証とビルド確認
  ...
  - L1→L2→L2.5→L3→L4の依存方向のみであることを検証
  - 循環依存が発生していないことを確認
  - _Requirements: 1.3_
...
- [x] 2.2 instruments/配下のアセットクラス別サブモジュール作成
...
- [x] 2.3 models/配下のモデルカテゴリ別サブモジュール作成
...
- [x] 3.2 CurveSet構造体の実装
  - 複数カーブを名前付きで管理するコンテナを実装
  - カーブの登録・取得メソッドを実装
  - ディスカウントカーブとフォワードカーブの分離取得を実装
  - _Requirements: 3.1, 3.2_

- [x] 3.3 CreditCurveトレイトとHazardRateCurve実装
...
- [x] 4.1 Instrumentトレイトの定義
  - 価格計算・Greeks計算・キャッシュフロー取得メソッドを定義
  - 満期・通貨取得メソッドを追加
  - 全商品が実装すべき共通インターフェースを確立
  - _Requirements: 2.3_

- [x] 4.2 InstrumentEnumの階層的Enum構造への再構成
...
- [x] 7.1 (P) CreditDefaultSwap構造体の実装
...
- [x] 7.3 デフォルトシミュレーションのMC拡張
  - 生存確率の逆関数でデフォルト時刻をサンプリング
  - MCエンジンにデフォルトイベントの処理追加
  - _Requirements: 7.4_
...
## Task 10: キャリブレーション基盤（pricer_optimiser）

- [x] 10.1 Levenberg-Marquardtソルバーの実装
  - LMソルバー構造体を定義
  - 非線形最小二乗法の反復計算を実装
  - 収束判定とパラメータ更新ロジックを実装
  - 数学ソルバーモジュールに追加
...
- [x] 10.4 SwaptionCalibratorの実装
  - Swaptionキャリブレータ構造体を定義
  - SwaptionボラティリティサーフェスへのキャリブレーションMを実装
  - Hull-Whiteモデルのパラメータ（平均回帰速度、ボラティリティ）を推定
  - _Requirements: 5.3_

- [x] 10.5 (P) BFGSソルバーの実装
  - BFGSソルバー構造体を定義
  - 準ニュートン法の反復計算を実装
  - _Requirements: 5.4_

- [ ] 10.6 Enzyme ADを活用した勾配計算
  - キャリブレーション目的関数のAD対応
  - 勾配計算によるキャリブレーション高速化
  - enzyme-modeでのテスト
  - _Requirements: 5.6_
...
- [ ] 12.1 (P) VarianceSwap構造体の実装
  - バリアンススワップ構造体を定義（ストライク、バリアンスノーショナル、観測日、smoothing_epsilon）
...
- [ ] 12.3 (P) Cliquet構造体の実装
  - クリケット構造体を定義（リセット日、ローカルキャップ/フロア、グローバルキャップ/フロア、smoothing_epsilon）
  - エキゾチック商品サブenumにCliquetバリアントを追加
  - _Requirements: 9.3_

- [ ] 12.4 (P) Autocallable構造体の実装
  - オートコーラブル構造体を定義（観測日、早期償還バリア、クーポン条件、ノックインプット、smoothing_epsilon）
  - エキゾチック商品サブenumにAutocallableバリアントを追加
  - _Requirements: 9.4_

- [ ] 12.5 (P) Rainbow（BestOf/WorstOf）構造体の実装
  - レインボー構造体を定義（原資産リスト、ペイオフタイプ、相関、smoothing_epsilon）
  - ペイオフタイプ列挙型を定義（BestOf, WorstOf, Spread, BasketCall, BasketPut）
  - エキゾチック商品サブenumにRainbowバリアントを追加
  - _Requirements: 9.5_

- [ ] 12.6 (P) QuantoOption構造体の実装
  - クオントオプション構造体を定義（原資産通貨、決済通貨、クオント調整、underlying_instrument_id）
  - Box使用を避けInstrumentIdによる参照方式を採用（Enzyme AD互換性維持）
...
以下の要件は本フェーズでは実装対象外とし、将来フェーズに延期：

- **Requirement 12**: 規制計算（SA-CCR, FRTB, SIMM）
  - 理由: 規制計算は独立したモジュールとして実装可能であり、コアプライシング機能の完成後に着手
